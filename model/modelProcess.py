        ...
        if process.poll() is None:
            ...
            for id in ids:
                ...
                if mcr.find_status(config.STATUS_LOCK):
                    mcr.update_status(config.STATUS_ERROR | config.STATUS_UNLOCK, 'Unexpected Error Happend!')
...
def daemon_process(process_queue: queue.Queue[any]):
    ...
    def update_task_queue():
        ...
        task_queue.intersection_update({ id for id in task_queue if not MCR.is_case_done(id) })
    ...
    def try_to_run_case(command):
        ...
        if len(task_queue) <= config.MAX_RUNNING_MODEL_CASE_NUM and id not in task_queue:
            task_queue.add(id)
    ...
    def ready_for_core_case(command: list[str]):
        ...
        for id in other_mcr_ids:
            current_status = MCR.get_case_status(id)
            if (current_status & config.STATUS_ERROR) == config.STATUS_ERROR:
                
                error_log = MCR.get_simplified_error_log(core_mcr_id)
                
                MCR.update_case_status(core_mcr_id, config.STATUS_ERROR | config.STATUS_UNLOCK, error_log, 'w')
                is_ready = 2
                break
...
class Process_Queue:
    ...
    def  initialize(self):
        ...
        daemon = multiprocessing.Process(target=daemon_process, args=(self.queue,), daemon=True)
        daemon.start()
